...
## پرامپت ۱۴۳
می‌خوام مشکل کندی در اسناد بزرگ را بر طرف کنی. این مشکل در افزونه `directionPlugin.js` است، این افزونه **جهت متن** (راست‌به‌چپ یا چپ‌به‌راست) را برای هر المان تنظیم می‌کنه. هر بار که رویداد `input` رخ می‌ده (یعنی کاربر یک حرف تایپ می‌کنه)، تمام المان‌های صفحه بررسی می‌شن. یکی از روش‌های زیر را استفاده کن:
1. فقط بلوکی که کاربر در آن تایپ می‌کنه را به‌روز بشه
2. به‌روزرسانی فقط المان‌های قابل مشاهده، از Intersection Observer استفاده کن
    - فقط المان‌هایی که در صفحه نمایش هستند را به‌روز کن